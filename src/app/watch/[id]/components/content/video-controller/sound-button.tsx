...
export default function SoundButton({videoEl}: {videoEl: HTMLVideoElement | null}) {
    ...
    onDidUpdate(function () {
        ...
        configSound(isMuted, volume);
    }, [videoEl]);

    const ontoggleMuted = function () {
        configSound(!isMuted, isMuted ? 1: 0);
    }

    const onchangeVolume = function ({target}: { target: HTMLInputElement}) {
        configSound(+target.value <= 0, +target.value);
    }

    function configSound(isMuted: boolean, volume: number) {
    ...
    return (
        <div className="volume-container">
            <button className="mute-btn" onClick={ontoggleMuted} data-volume-level={isMuted ? "low" : "high"}>
            ...
            <input  ref={volumeSliderRef} className="volume-slider w-0" type="range" min="0" max="1" step="any" value={volume} onChange={onchangeVolume}/>
        </div>
    )
}